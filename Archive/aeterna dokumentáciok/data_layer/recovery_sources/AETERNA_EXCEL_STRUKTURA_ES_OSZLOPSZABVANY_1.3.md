...
Current munkaforrás: `AETERNA – KÁRTYAADATBÁZIS MUNKAFORRÁS 1.9v.xlsx`
Kapcsolódó külön lookup-forrás: `LOOKUPS.xlsx`
...
Módosítás típusa: current workbook-szerkezet szinkron / 21 lapos munkaforrás / split LOOKUPS-réteg / ID- és delimiter-guard pontosítása

Érintett részek:

- current munkaforrás `1.9v`;
- 21 munkalapos szerkezet;
- `5A–5D` funkcionális LOOKUPS-lapok;
- `CARDS_MASTER` current fizikai `Card_ID` fejlécének dokumentálása;
- `Szabályi_Kártya_ID` / `Card_ID` migráció elhatárolása;
- runtime/canonical delimiter-policy;
- structured lookup-frissítési rend.

Státusz: aktív current-szerkezet szinkron.

Fontos:

- a 43 oszlopos `CARDS_MASTER` nem változik;
- a 22 oszlopos import/runtime nézet nem csonkolhatja a mastert;
- a munkaforrás beágyazott `5A. LOOKUPS_RUNTIME` lapja nem írhatja felül automatikusan a külön `LOOKUPS.xlsx` canonical runtime értékeit;
- ID- vagy delimiter-tömegmigráció nem része ennek a dokumentációs frissítésnek.
...
A current szerkesztési és auditforrás:

`AETERNA – KÁRTYAADATBÁZIS MUNKAFORRÁS 1.9v.xlsx`

Ez már nem egyszerű Birodalom-lapos `cards.xlsx`, hanem több munkalapos,
kibővített kártyaadatbázis-munkaforrás.

A fájl egyszerre szolgál:

- aktív kártyaadatbázis-munkaforrásként;
- kártyaauditálási munkafájlként;
- structured mező javítási felületként;
- döntés- és auditnaplóként;
...
- runtime / engine export előkészítő forrásként;
- névprofilozási és névaudit munkafelületként;
- több funkcionális LOOKUPS-réteg munkafájlbeli hordozójaként.

A munkaforrás **nem azonos** a production runtime authorityval.

Current adatút:

```text
emberi szerkesztési authority
AETERNA – KÁRTYAADATBÁZIS MUNKAFORRÁS 1.9v.xlsx + LOOKUPS.xlsx
→ export / validáció
→ CARDDATABASE.xlsx + REGISTRY.xlsx + canonical workbook export
→ runtime package
→ C# authoritative engine
```

A régi `cards.xlsx` legacy / összehasonlító / importforrás.
Nem production runtime authority.

A munkaforrás beágyazott LOOKUPS-lapjai munkafájl-validációs és
szerkezeti segédrétegek. A külön `LOOKUPS.xlsx` canonical szerepét
nem írhatják felül automatikusan.

## 3. A munkafájl current lapjai

Az `AETERNA – KÁRTYAADATBÁZIS MUNKAFORRÁS 1.9v.xlsx`
current állapotban **21 munkalapot** tartalmaz:
...
A `5A–5D` lapok a korábbi egyetlen LOOKUPS-réteg funkcionális szétválasztását adják:

- runtime structured értékek;
- print/product értékek;
- workflow/audit értékek;
- design catalog értékek.

A split lookup-lapok létrejötte current szerkezeti tény.

A külön `LOOKUPS.xlsx` ettől továbbra is önálló, ellenőrzendő lookup-forrás;
a workbook beágyazott lapjai és a külön lookup-forrás között eltérés esetén
nem végezhető automatikus felülírás.
...
- kitöltési szabályok;
- tipikus hibák;
- munkafolyamatbeli szerep.

## 4. Fontos szerkezeti pontosítás: 22 oszlopos szabvány és 43 oszlopos CARDS_MASTER

A régi / alap `cards.xlsx` struktúra, illetve bizonyos import-, runtime- vagy
exportcélú formátumok 22 oszlopos kártyaadat-szabványt használnak.

A current `3. CARDS_MASTER` ezzel szemben **43 oszlopos aktív kártyaadat-master**.

Ezért külön kell kezelni:

- a 22 oszlopos simplified / import / runtime / export adatnézetet;
- a 43 oszlopos `3. CARDS_MASTER` szerkesztési mastert.

A `CARDS_MASTER` nem csonkolható, nem generálható újra és nem vágható vissza
22 oszlopra csak azért, mert egy runtime/export nézet rövidebb.

A 10 kártyás emberi munkacsomag és a kétblokkos:

- 1–22. oszlop;
- 23–43. oszlop

módszer csak **presentation/workflow technika**.
A master logikailag továbbra is egyetlen 43 mezős rekord.

### 4.1 Current ID-contract eltérés

A current `1.9v` munkaforrás több aktív lapján a fizikai kártyaazonosító fejléc:

`Card_ID`

Más lapok, különösen printing/booster rétegek, továbbra is használhatják a:

`Szabályi_Kártya_ID`

elnevezést.

A dokumentációs/semantic cél továbbra is a stabil gameplay identity és a
külön printing identity szétválasztása:

- gameplay/szabályi identity;
- `Print_ID` printing/collector/art/treatment identity.

A `Card_ID` ↔ `Szabályi_Kártya_ID`, az `AET-` prefix és a legacy alias policy
**nincs lezárt tömeges migrációs állapotban**.

Ezért a jelen dokumentációs sync:

- nem nevez át workbook-oszlopot;
- nem generál új ID-kat;
- nem töröl aliasokat;
- nem módosít loader/export mappinget.

### 4.2 Current delimiter-policy

A runtime package current canonical többértékű delimiter-policyja:

`;`

A current workbookban azonban történetileg több structured mezőben vessző
is előfordul.

Ezért dokumentációs szinten a `;` a cél/canonical irány, de a workbook
tömeges delimiter-cseréje csak külön:

1. compatibility-audit;
2. exporter/parser ellenőrzés;
3. LOOKUPS összevetés;
4. kontrollált migráció;
5. runtime package regresszió

után végezhető.

A jelen 1.3 dokumentációs frissítés **nem módosít workbook-adatot**.
...
Card_ID
A current 1.9v `CARDS_MASTER` fizikai stabil kártyaazonosító mezője. Gameplay/szabályi identityként kezelendő, de a végleges `Card_ID` ↔ `Szabályi_Kártya_ID` mezőnév- és prefix-policy még külön migrációs döntés.
...
A gameplay-kártyaazonosító az elsődleges stabil identity; a current `CARDS_MASTER` fizikai fejléce `Card_ID`. A kártyanév változhat, az ID nem. A `Card_ID` / `Szabályi_Kártya_ID` átnevezést nem szabad implicit módon elvégezni.
...
a current fizikai `Card_ID` az elsődleges stabil gameplay-kártyaazonosító; a `Szabályi_Kártya_ID` név/prefix migráció külön döntés;
...
Cél: emberi munkafájl-validációs és összefoglaló lookup-réteg.

A lap továbbra is tartalmazhat olyan általános listákat, mint:
...
- audit státuszok;
- hibakategóriák;
- rarity/treatment/art/print/play-legal értékek.

Current szerepe:

- validációs segédlap;
- emberi áttekintő lookup-réteg;
- kompatibilitási/legacy összefoglaló.
...
- auditnapló;
- döntésnapló;
- önálló production runtime authority.

A current munkaforrásban a lookupok funkcionális split lapokra is szétváltak,
ezért új structured értéknél mindig azt kell ellenőrizni, **melyik lookup-domain**
a természetes cél.

### 7.6 5A. LOOKUPS_RUNTIME

Cél: runtime és gameplay structured értékek munkafájlbeli lookup-rétege.

Exportprofil:

`LOOKUPS_RUNTIME.jsonl`

Kötelező exportmezők:
...
- Effect-tag;
- Duration;
- Interpretation/status jellegű runtime structured csoportok.

Fontos authority-szabály:

A `5A. LOOKUPS_RUNTIME` munkafájl-validációs és történeti segédforrás.
**Nem írhatja felül automatikusan a külön `LOOKUPS.xlsx` aktív canonical runtime értékeit.**

Eltérés esetén külön lookup-audit szükséges.

### 7.7 5B. LOOKUPS_PRINT_PRODUCT

Cél: printing-, set-, product- és kapcsolódó kiadási lookupok munkafájlbeli rétege.

Exportprofil:

`LOOKUPS_PRINT_PRODUCT.jsonl`
...
- print status;
- art variant;
- set/product kategóriák;
- `Reprint_Of`-hoz kapcsolódó kontrollált értékek.

Ez a lap nem gameplay rules authority.

### 7.8 5C. LOOKUPS_WORKFLOW_AUDIT

Cél: munkafolyamat-, audit- és adatkarbantartási státuszok elkülönített lookup-rétege.

Exportprofil:

`LOOKUPS_WORKFLOW_AUDIT.jsonl`
...
- javítási prioritás;
- workflow státusz;
- döntési/audit segédérték.

Ez a lap nem runtime gameplay-mechanika.

### 7.9 5D. LOOKUPS_DESIGN_CATALOG

Cél: kártyatervezési, generálási és design-catalog kontrollált értékek elkülönített rétege.

Exportprofil:

`LOOKUPS_DESIGN_CATALOG.jsonl`
...
- `Generation_Profile`;
- design catalog elemek;
- generálási profilok;
- vizuális/printing design segédértékek.

Ez a lap nem gameplay rules authority és nem runtime legality engine.
...
Ha egy Birodalom vagy Klán auditja során új structured trigger, hatáscímke
vagy más kontrollált érték válik szükségessé, azt nem elég csak a
`CARDS_MASTER` egyes soraiban használni.

A munkafolyamat sorrendje:

1. Az érintett lapokon jelölni kell a structured hiányt vagy közelítő értéket.
2. Meg kell vizsgálni, hogy a probléma kártyaszöveg-hiba vagy structured értékkészlet-hiány.
3. Ha a kártyaszöveg működőképes, az új érték structured-bővítési jelöltként kezelhető.
4. A döntést szükség esetén `AUDIT_LOG` és `DECISION_LOG` bejegyzésben kell rögzíteni.
5. A `kartya_tabla_szabvany` dokumentumban át kell vezetni, ha elfogadott structured érték lesz.
6. Meg kell határozni a természetes lookup-domaint:
   - runtime/gameplay → `5A. LOOKUPS_RUNTIME`;
   - print/product → `5B. LOOKUPS_PRINT_PRODUCT`;
   - workflow/audit → `5C. LOOKUPS_WORKFLOW_AUDIT`;
   - design catalog → `5D. LOOKUPS_DESIGN_CATALOG`.
7. Ellenőrizni kell a külön `LOOKUPS.xlsx` canonical forrást is, ha a domain oda tartozik.
8. Csak authority- és compatibility-ellenőrzés után használhatják egységesen az érintett `CARDS_MASTER` sorok.
9. Ha engine-oldali feldolgozás kell, az `Engine_Gyanú` és `Engine_Megjegyzés` mezőkben is jelölni kell.
10. A `VERZIÓ` lapon rögzíteni kell a structured/lookup változást.
...
- `Audit_Státusz`;
- `Play_Legal_Status`;
- print/product státuszok;
- generation/design catalog értékek.

A structured értékkészlet bővítése önmagában nem módosítja a 43 oszlopos
`CARDS_MASTER` szerkezetét.

Authority-guard:

- a workbook `5A. LOOKUPS_RUNTIME` lapja nem írhatja felül automatikusan a külön `LOOKUPS.xlsx` canonical runtime értékeit;
- a külön `LOOKUPS.xlsx` eltéréseit nem szabad vakon visszaírni a workbookba;
- alias, új canonical érték és hibás/elírt érték között triázs szükséges.

Történeti példa a VENTUS / Égbolt Úrai auditból:
...
A példa azt mutatja, hogy egy új structured értéket a megfelelő szabványban,
lookup-domainben, érintett kártyasorokban és verzió/audit nyomvonalban
összehangoltan kell kezelni.

A jelen dokumentációs frissítés nem hajt végre workbook- vagy LOOKUPS-adatmigrációt.
