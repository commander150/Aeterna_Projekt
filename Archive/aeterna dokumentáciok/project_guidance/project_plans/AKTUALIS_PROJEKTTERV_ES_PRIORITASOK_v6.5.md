# AETERNA – AKTUÁLIS PROJEKTTERV ÉS PRIORITÁSOK v6.5

## VERZIÓ / DOKUMENTUMSTÁTUSZ

**Dokumentumverzió:** 6.5
**Dátum:** 2026-08-14
**Státusz:** aktív projektirányító és prioritási dokumentum
**Felváltott dokumentum:** `AKTUALIS_PROJEKTTERV_ES_PRIORITASOK_v6.4.md`
...
**Eltérés az előző checkpoint-bázishoz képest:** 39 commit
...
### Reaction / Priority Foundation v1 – ELŐKÉSZÍTÉS

**Státusz:** `NEXT – HUMAN/ASSISTANT CONTRACT WORK FIRST`

Nem indulhat vakon programozással.

Először:

1. a hivatalos `1.4.3v` reaction/timing szabályok célzott auditja;
2. az `OQ-LA-002`, `OQ-AR-005`, `OQ-ABIL-004` és kapcsolódó OQ-k aktualizálása;
3. a már eldöntött és még valóban nyitott részek szétválasztása;
4. explicit minimal Reaction / Priority contract készítése;
5. legal action, pending state, pass, event és resolution határ rögzítése;
6. csak ezután production C# implementáció.

A hivatalos forrás alapján már rögzített alapok közé tartozik:

- reaction window létrejötte meghatározott eseményeknél;
- a nem kezdeményező játékos első válaszlehetősége;
- pass;
- két egymást követő passz által lezárt ablak;
- egymásra épülő reakciók;
- visszafelé történő feloldási elv;
- lezárt eseményre nincs visszamenőleges reakció.

Továbbra is külön ellenőrzendő többek között:

- prevention/replacement;
- több trigger egyidejű sorrendje;
- optional/mandatory trigger viszony;
- pending choice és reaction egymásba ágyazása;
- action response és snapshot pontos reaction-state contractja.

Combat nem kerülhet be ebbe az első Reaction / Priority slice-ba.

---

## 6. Nyitott fő döntési kapuk
...
- blokkolás;
- Pecsétfeltörés;
- combat action/event contract.

Combat csak külön rules- és contract-előkészítés után indulhat.

### Reaction részletek

A reaction alapmodell már részben hivatalosan meghatározott, ezért az OQ-státuszokat újra kell auditálni. A teljes prevention/replacement/trigger-order rendszer nem tekintendő automatikusan lezártnak.

### Refresh Penalty

A production draw runtime jelenleg explicit módon nem implementálja a Refresh Penaltyt. Kötelező húzásnál a nem támogatott helyzet kontrolláltan elutasítható; a tényleges Refresh Penalty külön szabály- és implementation slice.
...
**Repository-bázis:** `2608345b61526097fc0b118f05461f92cfed0a95`
...
**Következő engine-fókusz:** Reaction / Priority Foundation v1 contract-előkészítés
**Combat:** még nem következő implementációs slice
**Dokumentációs consistency pass:** `COMPLETE` a `2608345b...` mérföldkőre

---

## 11. Következő szakmai munkasorrend

1. hivatalos `1.4.3v` Reaction / Priority timing audit;
2. a kapcsolódó Open Questions fennmaradó kapuinak pontosítása;
3. minimal Reaction / Priority Foundation v1 contract;
4. szükség esetén célzott clean-room learning ellenőrzés;
5. csak az elfogadott contract után Codex production implementáció;
6. implementáció után adversarial audit;
7. PASS után felhasználói commit/push;
8. következő valódi mérföldkőnél checkpoint-frissítés.

Combat, Pecsét és Refresh Penalty külön későbbi rules/contract kapu marad.
