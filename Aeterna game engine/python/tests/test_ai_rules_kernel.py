...
class TestAIRulesKernel(unittest.TestCase):
    ...
    def test_apply_end_turn_switches_active_player_and_logs_event(self):
        ...
        self.assertEqual(first_event["event_index"], 0)
        self.assertEqual(first_event["contract_type"], "engine_event")
        self.assertEqual(first_event["event_type"], "turn_transition")
        self.assertEqual(first_event["player_id"], "P1")
        self.assertEqual(first_event["action_type"], "end_turn")
        self.assertEqual(first_event["turn_number"], 1)
        self.assertEqual(first_payload["previous_active_player_id"], "P1")
        self.assertEqual(first_payload["next_active_player_id"], "P2")
        self.assertEqual(first_payload["turn_number_before"], 1)
        self.assertEqual(first_payload["turn_number_after"], 1)
        ...
        self.assertEqual(state.event_log[1]["event_type"], "turn_transition")
        self.assertEqual(state.event_log[1]["player_id"], "P2")
        self.assertEqual(state.event_log[1]["turn_number"], 2)
        self.assertEqual(state.event_log[1]["payload"]["turn_number_before"], 1)
        self.assertEqual(state.event_log[1]["payload"]["turn_number_after"], 2)
