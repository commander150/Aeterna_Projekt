...
class TestMinimalCardInstanceDrawMigration(unittest.TestCase):
    ...
    def test_duplicate_card_ids_remain_distinct_and_drawable(self):
        ...
        self.assertEqual(first_response["events"][0]["card_id"], first_card_id)
        self.assertEqual(second_response["events"][0]["card_id"], first_card_id)
    ...
    def test_draw_moves_one_instance_and_updates_registry_and_event(self):
        ...
        self.assertEqual(event["card_instance_id"], drawn_instance_id)
        self.assertEqual(event["card_id"], initial_record["card_id"])
        self.assertEqual(event["from_zone"], "deck")
        self.assertEqual(event["from_zone_index"], 0)
        self.assertEqual(event["to_zone"], "hand")
        self.assertEqual(event["to_zone_index"], len(initial_hand))
