    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        for field in profile.skip_row_if_empty_fields:
            if is_empty_value(raw_by_header.get(field)):
                warnings.append(f"{profile.name}: {row_number}. sor kihagyva: ĂĽres kĂ¶telezĹ‘ mezĹ‘: {field}")
                skipped = True
                break
        if skipped:
            continue
        ...
        for field in profile.required_fields:
            if field not in record or is_empty_value(record[field]):
                warnings.append(f"{profile.name}: {row_number}. sor: hiĂˇnyzĂł kĂ¶telezĹ‘ mezĹ‘: {field}")

        for field in number_fields:
            if field in record and record[field] != "none" and not isinstance(record[field], (int, float)):
                warnings.append(f"{profile.name}: {row_number}. sor: nem szĂˇmkĂ©nt Ă©rtelmezhetĹ‘ mezĹ‘: {field}={record[field]!r}")
...
def default_output_path(
    ...
    return Path(output_dir or DEFAULT_OUTPUT_DIR) / f"{source_name}__{sheet_part}.{output_format}"


def profile_output_path(profile: ExportProfile, output_dir: Path | None = None) -> Path:
    if profile.output_filename is None:
        raise ExportError("A generic_sheet profilhoz kĂ©zzel kell kimeneti fĂˇjlt megadni.")
    return Path(output_dir or DEFAULT_OUTPUT_DIR) / profile.output_filename
...
def resolve_export_options(
    ...
    if profile.generic:
        ...
        if missing:
            raise ExportError(f"A generic_sheet profilhoz kĂ¶telezĹ‘ argumentum(ok): {', '.join(missing)}")
        return profile, sheet_name, output_path, output_format

    if sheet_name is not None and sheet_name != profile.sheet_name:
        raise ExportError(f"A(z) {profile.name} profil lapja kĂ¶tĂ¶tt: {profile.sheet_name}")
    ...
    if resolved_format != profile.output_format:
        raise ExportError(f"A(z) {profile.name} profil kimeneti formĂˇtuma kĂ¶tĂ¶tt: {profile.output_format}")
    resolved_output = output_path or profile_output_path(profile, output_dir=output_dir)
...
def validate_profile_headers(headers: Sequence[str], profile: ExportProfile, warnings: list[str]) -> None:
    ...
    for field in profile.required_fields:
        if field not in header_set:
            raise ExportError(f"A(z) {profile.name} profilhoz hiĂˇnyzik a kĂ¶telezĹ‘ oszlop: {field}")
    for field in profile.number_fields:
        if field not in header_set:
            warnings.append(f"{profile.name}: hiĂˇnyzĂł szĂˇmmezĹ‘ oszlop: {field}")
...
def export_worksheet(
    ...
    if xlsx_path.resolve() == output_path.resolve():
        raise ExportError("A kimeneti fĂˇjl nem lehet azonos a forrĂˇs .xlsx fĂˇjllal.")
...
def find_xlsx_files(source_dir: Path | None = None) -> list[Path]:
    resolved_source_dir = source_dir or DEFAULT_SOURCE_DIR
    if not resolved_source_dir.exists():
        return []
    files = {path.resolve() for path in resolved_source_dir.glob("*.xlsx") if not path.name.startswith("~$")}
    return sorted(files, key=lambda path: path.name.casefold())


def validate_source_path(xlsx_path: Path, source_dir: Path | None = None) -> Path:
    resolved_path = xlsx_path.resolve()
    active_source_dir = source_dir
    if active_source_dir is None and SOURCE_DIR != DEFAULT_SOURCE_DIR:
        active_source_dir = SOURCE_DIR
    if active_source_dir is not None:
        active_source_path = active_source_dir.resolve() if hasattr(active_source_dir, "resolve") else Path(active_source_dir).resolve()
    if active_source_dir is not None and resolved_path.parent != active_source_path:
        raise ExportError(f"A forrĂˇs XLSX fĂˇjlnak ebben a mappĂˇban kell lennie: {active_source_dir}")
    return resolved_path


def choose_from_menu(title: str, options: Sequence[Any], label_func=str, input_func=input, print_func=print):
    if not options:
        raise ExportError(f"Nincs vĂˇlaszthatĂł elem: {title}")

    print_func(f"\n{title}")
    for index, option in enumerate(options, start=1):
        print_func(f"  {index}. {label_func(option)}")
    print_func("  0. KilĂ©pĂ©s")

    while True:
        answer = input_func("VĂˇlasztĂˇs: ").strip()
        if answer == "0":
            return None
        try:
        ...
        print_func("Ă‰rvĂ©nytelen vĂˇlasztĂˇs. Adj meg egy sorszĂˇmot.")


def interactive_menu(source_dir: Path | None = None, output_dir: Path | None = None, input_func=input, print_func=print) -> int:
    source_dir = Path(source_dir or DEFAULT_SOURCE_DIR)
    output_dir = Path(output_dir or DEFAULT_OUTPUT_DIR)
    print_func("XLSX EXPORT")
    print_func(f"ForrĂˇsmappa: {source_dir}")
    print_func(f"Kimeneti mappa: {output_dir}")

    xlsx_path = choose_from_menu(
        "VĂˇlaszd ki a forrĂˇs XLSX fĂˇjlt:",
        find_xlsx_files(source_dir),
    ...
    if profile_name == "generic_sheet":
        sheet_name = choose_from_menu(
            "VĂˇlaszd ki az exportĂˇlandĂł munkalapot:",
        ...
        output_format = choose_from_menu(
            "VĂˇlaszd ki a kimeneti formĂˇtumot:",
            ("jsonl", "tsv"),
            input_func=input_func,
            print_func=print_func,
        )
        if output_format is None:
            return 0
        output_path = default_output_path(xlsx_path, sheet_name, output_format, output_dir=output_dir)
    ...
    print_func(f"\nExportĂˇlt sorok szĂˇma: {count}")
    print_func(f"Kimeneti fĂˇjl: {output_path.resolve()}")
...
def print_warning_summary(warnings: Sequence[str], warnings_path: Path | None, print_func=print) -> None:
    ...
    if len(warnings) > 10:
        print_func(f"  ... tovĂˇbbi warningok: {len(warnings) - 10}")
    if warnings_path is not None:
        print_func(f"Warning fĂˇjl: {warnings_path.resolve()}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="XLSX munkalap exportĂˇlĂˇsa JSONL vagy TSV formĂˇtumba.")
    parser.add_argument("xlsx", nargs="?", type=Path, help="A forrĂˇs .xlsx fĂˇjl")
    parser.add_argument("--profile", choices=tuple(PROFILES), default="generic_sheet", help="Exportprofil")
    parser.add_argument("--list-sheets", action="store_true", help="A munkalapok kilistĂˇzĂˇsa")
    parser.add_argument("--sheet", help="Az exportĂˇlandĂł munkalap neve")
    parser.add_argument("--format", choices=("jsonl", "tsv"), dest="output_format", help="Kimeneti formĂˇtum")
    parser.add_argument(
        "--output",
        type=Path,
        help="Egyedi kimeneti fĂˇjl. Ha nincs megadva, az exports mappĂˇba kĂ©szĂĽl automatikus nĂ©vvel.",
    )
    parser.add_argument(
        "--source-dir",
        type=Path,
        default=None,
        help="Optional source directory used by interactive mode and optional source validation.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_OUTPUT_DIR,
        help="Default output directory for profile and automatic generic exports.",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.xlsx is None:
            return interactive_menu(source_dir=args.source_dir, output_dir=args.output_dir)
        args.xlsx = validate_source_path(args.xlsx, source_dir=args.source_dir)
        if not args.xlsx.is_file():
            raise ExportError(f"A forrĂˇsfĂˇjl nem talĂˇlhatĂł: {args.xlsx}")
        if args.xlsx.suffix.lower() != ".xlsx":
            raise ExportError("A forrĂˇsfĂˇjlnak .xlsx kiterjesztĂ©sĹ±nek kell lennie.")
        ...
        if args.profile == "generic_sheet" and args.output is None and args.sheet and args.output_format:
            args.output = default_output_path(args.xlsx, args.sheet, args.output_format, output_dir=args.output_dir)
        ...
        warnings_path = write_warnings(output_path, warnings)
        print(f"ExportĂˇlt sorok szĂˇma: {count}")
        print(f"Kimeneti fĂˇjl: {output_path.resolve()}")
        ...
        print(f"Hiba az XLSX fĂˇjl feldolgozĂˇsakor: {exc}", file=sys.stderr)
        return 1
