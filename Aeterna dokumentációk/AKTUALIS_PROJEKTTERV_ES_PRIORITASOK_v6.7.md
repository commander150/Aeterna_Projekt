# AETERNA – AKTUÁLIS PROJEKTTERV ÉS PRIORITÁSOK v6.7

## VERZIÓ / DOKUMENTUMSTÁTUSZ

**Dokumentumverzió:** 6.7
**Dátum:** 2026-08-16
**Státusz:** aktív projektirányító és prioritási dokumentum
**Felváltott dokumentum:** `AKTUALIS_PROJEKTTERV_ES_PRIORITASOK_v6.6.md`
...
**Contract:** `Aeterna game engine/docs/REACTION_PRIORITY_CONTRACT.md`
**Contract státusz:** `ACCEPTED_FOR_IMPLEMENTATION`
**Pre-implementation audit:** `PASS_WITH_RC1_CORRECTIONS`
**Implementation:** `NOT_STARTED / NEXT`

Az általános Reaction rules/source/OQ/research kör lezárult az első v1 slice-hoz.

Freeze-gate:

1. underlying canonical ability resolution a stack alján;
2. `ReactionSubjectId` külön az `EngineEvent.EventId` fogalomtól;
3. exact `ReactionWindowState` minimum contract;
4. canonical ability resolution stack-entry minimum contract;
5. typed response-policy registry;
6. RC1 single-responder lifecycle;
7. RC2 queue + post-resolution checkpoint + first-slice trigger boundary;
8. compound/nested choice/window kizárása az első slice-ból.
...
Combat, Pecsét, Refresh Penalty, generic prevention/replacement,
Burst/Jel full support és multi-trigger order-choice külön későbbi slice.
...
1. elfogadott `REACTION_PRIORITY_CONTRACT.md` alapján Codex production implementation;
2. Debug/Release build és teljes teszt;
3. determinism/reference regression;
4. Godot positive/negative production smoke;
5. adversarial read-only audit;
6. hibajavítás, ha szükséges;
7. PASS után felhasználói commit/push;
8. remote verification;
9. következő production mérföldkő dokumentációs lezárása;
10. ezután külön Combat rules/contract slice.

Új általános Reaction-kutatási kör az első slice előtt nem szükséges.
